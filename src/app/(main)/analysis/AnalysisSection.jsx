...
const AnalysisSection = () => {
    ...
    const organizeData = (dataDetail, data) => {
      return dataDetail.map((row) => {
        ...
        const acc = parseFloat(
          row.cells.find((cell) => cell.columnId === "acc").text
        );
        const price = parseFloat(
          row.cells.find((cell) => cell.columnId === "price").text
        );
  ...
  const filteredData = organizedData.filter((item) => item.acc && item.price); 

  const chartData = {
    labels: [...new Set(filteredData.map((item) => item.activeType))], 
    datasets: [
      {
        data: filteredData
          .reduce((acc, item) => {
            const typeIndex = acc.findIndex(
              (entry) => entry.type === item.activeType
            );
  ...
  <tbody>
    {organizedData.length > 0 ? (
      organizedData.map((data) =>
        // Verificamos que 'acc' y 'price' existan
        data.acc && data.price ? (
          <tr key={data.name}>
          ...
          // En caso de que falten datos, podrías mostrar una fila vacía o personalizada.
          <tr key={data.name}>
            <td colSpan={4} className="p-3 text-center text-red-500">
              Datos incompletos para {data.name}
            </td>
          </tr>
        )
      )
